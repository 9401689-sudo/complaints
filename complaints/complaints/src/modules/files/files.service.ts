...
export class FilesService {
  ...
  async syncCaseFiles(caseId: string) {
    ...
    try {
      ...
      const nextState =
        selectedFiles.length > 0 && files.length > 0 ? 'files_selected' : 'files_synced';

      const fsm = await fsmService.transition(caseId, nextState, {
  ...
  async updateSelectedFiles(caseId: string, body: UpdateCaseFilesBody): Promise<UpdateCaseFilesResult> {
    ...
    try {
      ...
      await fsmService.transition(caseId, nextState, {
        filesTotal: files.length,
        filesSelected: selectedFiles.length,
        selectedFileIds,
        textReady: false,
